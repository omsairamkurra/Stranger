module.exports=function({app,dbConn,upload}){
    app.post('./users/create',upload.single("avatar"),(req,res,next)=>{
        const file=req.file
        if(!file){
            res.status(200).jsonp({message:"Please upload your avatar"})
        }else{
                ...
                dbConn.query(findAccountByEmail,[email],function(error,account){
                    if(account&&account.length !==0){
                        ...
                        dbConn.query(registerUserSql,[users],function(error,insertedUser){
                            if(insertedUser){
                                res.status(200).jsonp({avatar,insertId:insertedUser})
                            }else{
    ...
    app.post('/users/followers',(req,res)=>{
        ...
        const updatedNumberOfFollowersSql="UPDATE user_account SET user_number_of_followers=? WHERE id=?"
        dbConn.query(updatedNumberOfFollowersSql,[numberOfFollowers,id],function(err,updatedUser){
            if(err){
    ...
    app.post('/users/posts',(req,res)=>{
        ...
        const updatedNumberOfPostsSql="UPDATE user_account SET user_number_of_posts=? WHERE id=?"
        dbConn.query(updatedNumberOfPostsSql,[numberOfPosts,id],function(err,updatedUser){
            if(err){
                res.status(200).jsonp({message:"The System error.Please try again"})
            }else if(updatedUser){
                res.status(200).jsonp({id})
